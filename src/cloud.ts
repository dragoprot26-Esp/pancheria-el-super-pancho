 ...
 *
 * cloud.ts — Capa de nube (Supabase) para Panchería El Súper Pancho.
...
export async function asegurarCuentaSeguraColab(usuario: string, password: string, codigo: string) {
  ...
  if (!sess) {
    let ok = false;
    try { ok = await rpc('panc_verificar_colab', { p_codigo: codigo, p_usuario: usuario, p_pass: password }, false); }
    catch (e) { ok = false; }
  ...
  try { await rpc('panc_unir_colab', { p_codigo: codigo, p_usuario: usuario }); }
...
// ── Sync de datos del local (tabla panc_backups, RLS por membresía) ────
export async function cloudLoad(codigo: string): Promise<CloudData | null> {
  const bearer = (await authToken()) || SB_KEY;
  try {
    const res = await fetch(
      `${SB_URL}/rest/v1/panc_backups?tenant_id=eq.${encodeURIComponent(codigo)}&select=datos&limit=1`,
...
export async function cloudSave(codigo: string, datos: CloudData): Promise<boolean> {
  const bearer = (await authToken()) || SB_KEY;
  try {
    const res = await fetch(`${SB_URL}/rest/v1/panc_backups`, {
      method: 'POST',
      headers: {
...
export async function pancPublica(codigo: string): Promise<CloudData | null> {
  try { return await rpc('panc_publica', { p_codigo: codigo }, false) as CloudData; }
  catch (e) { return null; }
}

export async function pancAgregarPedido(codigo: string, pedido: any): Promise<void> {
